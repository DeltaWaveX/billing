...
export default function RetailReportPage() {
  ...
  const loadBills = async () => {
    try {
      ...
      const retailBills = data.filter(b => b.type === 1)
      setBills(retailBills)
      setError(null)
    } catch (e) {
      console.error(e)
      setError("Failed to load retail bills.")
    } finally {
      setLoading(false)
    }
  }
          ...
          <Table>
            ...
            <TableBody>
              {filteredBills.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-6 text-muted-foreground">No retail bills found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
